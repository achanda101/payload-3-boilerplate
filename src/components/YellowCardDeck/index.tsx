...
export const YellowCardDeck: React.FC<YellowCardDeckProps> = ({ title, desc, cards, align }) => {
  ...
  return (
    <>
      {title && (
        <Heading
          level={3}
          className={`${align === 'left' ? 'col-span-6 text-left' : 'col-span-full text-center'}`}
        >
          {title}
        </Heading>
      )}
      {desc && typeof desc === 'object' ? (
        <div
          className={`${align === 'left' ? 'col-start-1 col-span-6 text-left' : 'col-span-full text-center'} -mt-6`}
        >
          {serializeLexical({ nodes: desc.root?.children || [] })}
        </div>
      ) : (
        <p
          className={`${align === 'left' ? 'col-span-6 text-left' : 'col-span-full text-center'} -mt-6`}
        >
          {desc}
        </p>
      )}
        ...
        <div className="hidden md:block relative">
          ...
          >
            <CarouselContent className="-ml-4">
              {cards?.map((card) => (
                <CarouselItem key={card.id} className="pl-4 mx-auto basis-auto">
                  <div className="turmeric_wavycard">
                    ...
                    >
                      {card.mascot && (
                        <div
                          className={`${card.mascotPos === 'top_left' ? 'p-0' : 'flex justify-center'}`}
                        >
                          <Image
                            src={card.mascot.url || ''}
