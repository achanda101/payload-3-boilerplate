...
export const GrantCardGrid: React.FC<GrantCardGridProps> = ({ title, desc, grantCards }) => {
  const colouredCards = grantCards?.grantCardGrid?.filter(
    (card) => card.cardColour && card.cardColour !== 'trans',
  )
  const transparentCards = grantCards?.grantCardGrid?.filter(
    (card) => card.cardColour && card.cardColour === 'trans',
  )
    ...
    <>
      ...
      {colouredCards?.length > 0
        ? colouredCards.map((card, index) => {
            const isLastOddCard =
              index === colouredCards.length - 1 && colouredCards.length % 2 === 1

            if (isLastOddCard) {
              return (
                <div
                  key={index}
                  className="col-span-6 md:col-span-4 md:col-start-3 lg:col-span-6 lg:col-start-4"
                >
                  <GrantCard {...card} />
                </div>
              )
        ...
        : null}
      {transparentCards?.length > 0
        ? transparentCards.map((card, index) => {
            const isLastOddCard =
              index === transparentCards.length - 1 && transparentCards.length % 2 === 1
