...
): Promise<{ types: DocType[]; mmediaposts: any[] }> {
  try {
    const draftParam = isDraft ? '&draft=true' : ''

    const response = await fetch(
      `/api/${collection}?limit=1000&locale=${locale}&depth=3&where[pageType][equals]=individual&sort=-publishedAt${draftParam}`,
    )
    const data = await response.json()

    if (!data.docs || !Array.isArray(data.docs)) {
      return { types: [], mmediaposts: [] }
    }
    ...
    return { types, mmediaposts: data.docs }
  } catch (error) {
...
export const MMediaPage: React.FC<MMediaPageProps> = ({ collection, docId, isDraft = false, locale = 'en', initialData }) => {
  ...
  const handleLanguageChange = useCallback(
    async (newLanguage: string) => {
      ...
      try {
        ...
        console.error('Failed to fetch Hero and Content Blocks data on Multimedia Page:', error)
      }
    },
  ...
  useEffect(() => {
    const fetchResourceTypes = async () => {
      // Only fetch mmediaposts if showFilter is enabled
      if (heroBlock?.showFilter) {
        const { types, mmediaposts } = await getAllResourceTypesFromCollection(
          collection,
          selectedLanguage,
          isDraft,
        )
        setResourceTypes(types)
        setAllMmediaposts(mmediaposts)
      }
    }
    fetchResourceTypes()
  ...
  return (
    <>
        ...
        >
          ...
          {heroBlock?.showFilter && (
            <div className="hero-content !gap-[0.25rem] !mt-[2rem] pill-outline">
              <p className="small">Multimedia Post Type</p>
      ...
      {/* Multimedia Posts Grid Section - Only show on landing page */}
      {pageType === 'landing' && filteredMmediaposts.length > 0 && (
        <div className="w-full px-[1.25rem] lg:px-[5rem] py-12">
          ...
          >
            {filteredMmediaposts.slice(0, displayCount).map((mmediapost: any) => {
              ...
              const tags =
                mmediapost.docType && Array.isArray(mmediapost.docType)
                  ? mmediapost.docType.map((type: any) => ({
                      id: type.id || '',
                      tag: type.type || '',
                    }))
                  : []
              ...
              return (
                <ResourceCard
                  key={mmediapost.id}
                  id={mmediapost.id}
                  title={mmediapost.title}
                  desc={mmediapost.heroSubtitle || null}
                  image={mmediapost.image}
                  tags={tags}
                  link={mmediapostLink}
                />
              )
